...
DeathConfirm=False #Boolean value used to confirm the death of the player when it enders the death state
...
class Player(GravityActor):
    ...
    def update(self):
        ...
        if self.hurt_timer > 100:
            ...
            if game.fruits and self.State == PlayerStates.Collect and DeathConfirm == False:
                #print("Collecting")
                ...
                if self.y==game.fruits[0].y:#If our Y position is the same as the fruits move left or right untill you reach the fruit
                    ...
                    if self.Change == False:
                     ...
                     if self.Change == False:
                        ...
                        dx = 1
##                     if self.x == game.fruits[0].x and self.vel_y == 0 and self.landed: #if while our y position is less than the fruits but our x is the same jump to reach fruit
##                        print("Jumping")
##                        self.vel_y = -16
##                        self.landed = False
##                        game.play_sound("jump")
            ...
            #And We are alive
            if game.bolts and self.y-38 == game.bolts[0].y and DeathConfirm == False:
                print("Defending")
            ...
            if game.enemies and DeathConfirm == False and not game.fruits:
                    print("Attack")
            ...
            if self.State == PlayerStates.Attack:
                    if game.fruits:#In case there are fruits that have spawned in return to collecting it
                         ...
                         self.State = PlayerStates.Collect
                    if game.enemies and self.x<game.enemies[0].x and self.x!=game.enemies[0].x:#Truck The enemies X
                            dx = 1
                    elif game.enemies and self.x>game.enemies[0].x and self.x!=game.enemies[0].x:#Truck the enemies X
                            dx = -1
                    if game.enemies and game.enemies[0].y  == self.y:
                        if self.x<game.enemies[0].x and game.enemies:#If the bolt is coming from the right fire a bublle right
                            dx = 1
                            if self.fire_timer <= 0 and len(game.orbs) < 5:#Check if the minimy time limit has passed and there are only 5 orbs generated
                                x = min(730, max(70, self.x + self.direction_x * 38))
                                y = self.y - 35
                            ...
                            if self.blowing_orb:
                                ...
                                if self.blowing_orb.blown_frames >= 120:
                                    # Can't be blown any further
                                    self.blowing_orb = None
                        elif game.enemies and self.x>game.enemies[0].x:#If the bolt is coming from the right fire a bublle right
                            dx = -1
...
def update():
    global state, game,DeathConfirm
    ...
    if state == State.MENU:
        ...
        if DeathConfirm == True:#If DeathConfirm equals True end the game
